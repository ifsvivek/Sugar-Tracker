<script>
	...
	$: hasData = sugarData && sugarData.length > 0 && sugarData.some(val => val > 0);
	$: hasMealData = mealBreakdown && mealBreakdown.length > 0 && mealBreakdown.some(meal => meal.sugar > 0);

	// Calculate average values
	$: avgSugar =
		sugarData.length > 0 && hasData
	...
	$: maxSugar = Math.max(...(hasData ? sugarData : [0]), sugarGoal, 1) * 1.2;
		...
		datasets: [
			{
				label: 'Sugar (g)',
				data: sugarData,
				backgroundColor: sugarData.map((value) =>
					value > sugarGoal ? 'rgba(239, 68, 68, 0.7)' : 'rgba(16, 185, 129, 0.7)'
				),
				borderColor: sugarData.map((value) =>
					value > sugarGoal ? 'rgb(220, 38, 38)' : 'rgb(4, 120, 87)'
				),
				borderWidth: 1,
				borderRadius: 4
			}
	...
	$: mealChartOptions = {
		...
		plugins: {
			...
			tooltip: {
				callbacks: {
					label: (item) => `${item.raw}g (${mealBreakdown[item.dataIndex]?.percentage || 0}%)`
				}
			}
		}
	};
