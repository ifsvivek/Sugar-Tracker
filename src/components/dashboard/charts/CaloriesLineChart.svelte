<script>
	...
	$: hasCalorieData = calorieData && calorieData.length > 0 && calorieData.some(val => val > 0);
	$: hasMacroData = (carbsData && carbsData.length > 0 && carbsData.some(val => val > 0)) ||
		(proteinData && proteinData.length > 0 && proteinData.some(val => val > 0)) ||
		(fatData && fatData.length > 0 && fatData.some(val => val > 0));
	$: hasTodayData = totalCarbs > 0 || totalProtein > 0 || totalFat > 0;

	// Calculate average values
	$: avgCalories =
		calorieData?.length > 0 && hasCalorieData
...
<div class="mb-6 space-y-4">
	...
	{#if hasTodayData}
		<div class="mt-4 grid grid-cols-4 gap-4">
			<div
				class="rounded-lg bg-blue-50 p-3 text-center transition-all duration-200 hover:-translate-y-1 hover:transform hover:shadow-md"
			>
				<p class="text-xs text-gray-500">From Carbs</p>
				<p class="mt-1 font-medium text-blue-700">{carbsCalories} cal</p>
			</div>
			<div
				class="rounded-lg bg-green-50 p-3 text-center transition-all duration-200 hover:-translate-y-1 hover:transform hover:shadow-md"
			>
				<p class="text-xs text-gray-500">From Protein</p>
				<p class="mt-1 font-medium text-green-700">{proteinCalories} cal</p>
			</div>
			<div
				class="rounded-lg bg-purple-50 p-3 text-center transition-all duration-200 hover:-translate-y-1 hover:transform hover:shadow-md"
			>
				<p class="text-xs text-gray-500">From Fat</p>
				<p class="mt-1 font-medium text-purple-700">{fatCalories} cal</p>
			</div>
			<div
				class="rounded-lg bg-gray-50 p-3 text-center transition-all duration-200 hover:-translate-y-1 hover:transform hover:shadow-md"
			>
		...
		<div class="mt-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
			<h4 class="mb-4 text-center text-sm font-medium text-gray-700">Today's Calorie Sources</h4>
			<div class="h-60">
				<ChartJsWrapper
					type="doughnut"
					data={macroChartData}
	...
	{:else}
		<div class="mt-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
			<div class="flex flex-col items-center justify-center py-8">
				<p class="text-gray-400">Log your meals to see calorie breakdown</p>
			</div>
		</div>
	{/if}
</div>
